...
function renderFilterChips(
  ...
  const daysLabel =
    selectedDays.length === 1
      ? selectedDays[0]
      : tr("usageOverview.daysN", "{count} days", { count: String(selectedDays.length) });
  const hoursLabel =
    selectedHours.length === 1
      ? `${selectedHours[0]}:00`
      : tr("usageOverview.hoursN", "{count} hours", { count: String(selectedHours.length) });
    ...
    <div class="active-filters">
        ...
        selectedDays.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${tr("usageOverview.days", "Days")}: ${daysLabel}</span>
              <button class="filter-chip-remove" @click=${onClearDays} title=${tr("usagePage.removeFilter", "Remove filter")}>×</button>
            </div>
          `
          : nothing
      }
      ${
        selectedHours.length > 0
          ? html`
            <div class="filter-chip">
              <span class="filter-chip-label">${tr("usageOverview.hours", "Hours")}: ${hoursLabel}</span>
              <button class="filter-chip-remove" @click=${onClearHours} title=${tr("usagePage.removeFilter", "Remove filter")}>×</button>
            </div>
          `
          : nothing
      }
        ...
        selectedSessions.length > 0
          ? html`
            <div class="filter-chip" title="${sessionsFullName}">
              <span class="filter-chip-label">${tr("usageOverview.session", "Session")}: ${sessionsLabel}</span>
              <button class="filter-chip-remove" @click=${onClearSessions} title=${tr("usagePage.removeFilter", "Remove filter")}>×</button>
            </div>
          `
          : nothing
      }
        ...
        (selectedDays.length > 0 || selectedHours.length > 0) && selectedSessions.length > 0
          ? html`
            <button class="btn btn-sm filter-clear-btn" @click=${onClearFilters}>
              ${tr("usageOverview.clearAll", "Clear All")}
            </button>
          `
          : nothing
      }
  ...
  if (!daily.length) {
    return html`
      <div class="daily-chart-compact">
        <div class="sessions-panel-title">${tr("usageOverview.dailyUsage", "Daily Usage")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${tr("usageOverview.noData", "No data")}</div>
      </div>
    `;
  }
    ...
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        <div class="chart-toggle small sessions-toggle">
          ...
          >
            ${tr("usageOverview.total", "Total")}
          </button>
          <button
            class="toggle-btn ${dailyChartMode === "by-type" ? "active" : ""}"
            @click=${() => onDailyChartModeChange("by-type")}
          >
            ${tr("usageOverview.byType", "By Type")}
          </button>
        </div>
        <div class="card-title">${tr("usageOverview.daily", "Daily")} ${isTokenMode ? tr("usageOverview.token", "Token") : tr("usageOverview.cost", "Cost")} ${tr("usageOverview.usage", "Usage")}</div>
      </div>
          ...
          ${daily.map((d, idx) => {
            ...
            const breakdownLines =
              dailyChartMode === "by-type"
                ? isTokenMode
                  ? [
                      `${tr("usageOverview.output", "Output")} ${formatTokens(d.output)}`,
                      `${tr("usageOverview.input", "Input")} ${formatTokens(d.input)}`,
                      `${tr("usageOverview.cacheWrite", "Cache write")} ${formatTokens(d.cacheWrite)}`,
                      `${tr("usageOverview.cacheRead", "Cache read")} ${formatTokens(d.cacheRead)}`,
                    ]
                  : [
                      `${tr("usageOverview.output", "Output")} ${formatCost(d.outputCost ?? 0)}`,
                      `${tr("usageOverview.input", "Input")} ${formatCost(d.inputCost ?? 0)}`,
                      `${tr("usageOverview.cacheWrite", "Cache write")} ${formatCost(d.cacheWriteCost ?? 0)}`,
                      `${tr("usageOverview.cacheRead", "Cache read")} ${formatCost(d.cacheReadCost ?? 0)}`,
                    ]
                : [];
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? tr("usageOverview.tokens", "Tokens") : tr("usageOverview.cost", "Cost")} ${tr("usageOverview.byType", "by Type")}</div>
      ...
      <div class="cost-breakdown-total">
        ${tr("usageOverview.total", "Total")}: ${isTokenMode ? formatTokens(totals.totalTokens) : formatCost(totals.totalCost)}
      </div>
    </div>
  `;
}
...
function renderUsageInsights(
  ...
  const errorDays = aggregates.daily
    ...
    .map((day) => {
      ...
      return {
        ...
        sub: `${day.errors} ${tr("usageOverview.errors", "errors")} · ${day.messages} ${tr("usageOverview.msgs", "msgs")} · ${formatTokens(day.tokens)}`,
        rate,
      };
    })
  ...
  const topModels = aggregates.byModel.slice(0, 5).map((entry) => ({
    label: entry.model ?? tr("usageOverview.unknown", "unknown"),
    value: formatCost(entry.totals.totalCost),
    sub: `${formatTokens(entry.totals.totalTokens)} · ${entry.count} ${tr("usageOverview.msgs", "msgs")}`,
  }));
  const topProviders = aggregates.byProvider.slice(0, 5).map((entry) => ({
    label: entry.provider ?? tr("usageOverview.unknown", "unknown"),
    value: formatCost(entry.totals.totalCost),
    sub: `${formatTokens(entry.totals.totalTokens)} · ${entry.count} ${tr("usageOverview.msgs", "msgs")}`,
  }));
  const topTools = aggregates.tools.tools.slice(0, 6).map((tool) => ({
    label: tool.name,
    value: `${tool.count}`,
    sub: tr("usageOverview.calls", "calls"),
  }));
  ...
  return html`
    <section class="card" style="margin-top: 16px;">
      <div class="card-title">${tr("usageOverview.usageOverview", "Usage Overview")}</div>
      <div class="usage-summary-grid">
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.messages", "Messages")}
            <span class="usage-summary-hint" title=${tr("usageOverview.hintTotalMessages", "Total user + assistant messages in range.")}>?</span>
          </div>
          <div class="usage-summary-value">${aggregates.messages.total}</div>
          <div class="usage-summary-sub">
            ${aggregates.messages.user} ${tr("usageDetails.user", "user")} · ${aggregates.messages.assistant} ${tr("usageDetails.assistant", "assistant")}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageDetails.toolCalls", "Tool Calls")}
            <span class="usage-summary-hint" title=${tr("usageOverview.hintToolCalls", "Total tool call count across sessions.")}>?</span>
          </div>
          <div class="usage-summary-value">${aggregates.tools.totalCalls}</div>
          <div class="usage-summary-sub">${aggregates.tools.uniqueTools} ${tr("usageOverview.toolsUsed", "tools used")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.errors", "Errors")}
            <span class="usage-summary-hint" title=${tr("usageOverview.hintErrors", "Total message/tool errors in range.")}>?</span>
          </div>
          <div class="usage-summary-value">${aggregates.messages.errors}</div>
          <div class="usage-summary-sub">${aggregates.messages.toolResults} ${tr("usageOverview.toolResults", "tool results")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.avgTokensPerMsg", "Avg Tokens / Msg")}
            <span class="usage-summary-hint" title=${tokensHint}>?</span>
          </div>
          <div class="usage-summary-value">${formatTokens(avgTokens)}</div>
          <div class="usage-summary-sub">${tr("usageOverview.acrossMessages", "Across {count} messages", { count: String(aggregates.messages.total || 0) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.avgCostPerMsg", "Avg Cost / Msg")}
            <span class="usage-summary-hint" title=${costHint}>?</span>
          </div>
          <div class="usage-summary-value">${formatCost(avgCost, 4)}</div>
          <div class="usage-summary-sub">${formatCost(totals.totalCost)} ${tr("usageOverview.total", "total")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.sessions", "Sessions")}
            <span class="usage-summary-hint" title=${tr("usageOverview.hintDistinctSessions", "Distinct sessions in the range.")}>?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">${tr("usageOverview.ofInRange", "of {count} in range", { count: String(totalSessions) })}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.throughput", "Throughput")}
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.errorRate", "Error Rate")}
          ...
          <div class="usage-summary-sub">
            ${aggregates.messages.errors} ${tr("usageOverview.errors", "errors")} · ${avgDurationLabel} ${tr("usageOverview.avgSession", "avg session")}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${tr("usageOverview.cacheHitRate", "Cache Hit Rate")}
          ...
          <div class="usage-summary-sub">
            ${formatTokens(totals.cacheRead)} ${tr("usageOverview.cached", "cached")} · ${formatTokens(cacheBase)} ${tr("usageOverview.prompt", "prompt")}
          </div>
        </div>
      </div>
      <div class="usage-insights-grid">
        ${renderInsightList(tr("usageOverview.topModels", "Top Models"), topModels, tr("usageOverview.noModelData", "No model data"))}
        ${renderInsightList(tr("usageOverview.topProviders", "Top Providers"), topProviders, tr("usageOverview.noProviderData", "No provider data"))}
        ${renderInsightList(tr("usageDetails.topTools", "Top Tools"), topTools, tr("usageDetails.noToolCalls", "No tool calls"))}
        ${renderInsightList(tr("usageOverview.topAgents", "Top Agents"), topAgents, tr("usageOverview.noAgentData", "No agent data"))}
        ${renderInsightList(tr("usageOverview.topChannels", "Top Channels"), topChannels, tr("usageOverview.noChannelData", "No channel data"))}
        ${renderPeakErrorList(tr("usageOverview.peakErrorDays", "Peak Error Days"), errorDays, tr("usageOverview.noErrorData", "No error data"))}
        ${renderPeakErrorList(tr("usageOverview.peakErrorHours", "Peak Error Hours"), errorHours, tr("usageOverview.noErrorData", "No error data"))}
      </div>
    </section>
  `;
}
      ...
      >
        ...
        <div class="session-bar-actions">
          <button
            ...
            title=${tr("usageOverview.copySessionName", "Copy session name")}
            @click=${(e: MouseEvent) => {
              e.stopPropagation();
              void copySessionName(s);
            }}
          >
            ${tr("usageOverview.copy", "Copy")}
          </button>
  ...
  return html`
    <div class="card sessions-card">
      <div class="sessions-card-header">
        <div class="card-title">${tr("usageOverview.sessions", "Sessions")}</div>
        <div class="sessions-card-count">
          ${sessions.length} ${tr("usageOverview.shown", "shown")}${totalSessions !== sessions.length ? ` · ${totalSessions} ${tr("usageOverview.total", "total")}` : ""}
        </div>
      </div>
      <div class="sessions-card-meta">
        <div class="sessions-card-stats">
          <span>${isTokenMode ? formatTokens(avgValue) : formatCost(avgValue)} ${tr("usageOverview.avg", "avg")}</span>
          <span>${totalErrors} ${tr("usageOverview.errors", "errors")}</span>
        </div>
        <div class="chart-toggle small">
          ...
          >
            ${tr("usageOverview.all", "All")}
          </button>
          <button
            class="toggle-btn ${sessionsTab === "recent" ? "active" : ""}"
            @click=${() => onSessionsTabChange("recent")}
          >
            ${tr("usageOverview.recentlyViewed", "Recently viewed")}
          </button>
        </div>
        <label class="sessions-sort">
          <span>${tr("usageOverview.sort", "Sort")}</span>
          <select
            @change=${(e: Event) => onSessionSortChange((e.target as HTMLSelectElement).value as typeof sessionSort)}
          >
            <option value="cost" ?selected=${sessionSort === "cost"}>${tr("usageOverview.cost", "Cost")}</option>
            <option value="errors" ?selected=${sessionSort === "errors"}>${tr("usageOverview.errors", "Errors")}</option>
            <option value="messages" ?selected=${sessionSort === "messages"}>${tr("usageOverview.messages", "Messages")}</option>
            <option value="recent" ?selected=${sessionSort === "recent"}>${tr("usageOverview.recent", "Recent")}</option>
            <option value="tokens" ?selected=${sessionSort === "tokens"}>${tr("usageOverview.tokens", "Tokens")}</option>
          </select>
        </label>
        <button
          class="btn btn-sm sessions-action-btn icon"
          @click=${() => onSessionSortDirChange(sessionSortDir === "desc" ? "asc" : "desc")}
          title=${sessionSortDir === "desc" ? tr("usageOverview.desc", "Descending") : tr("usageOverview.asc", "Ascending")}
        >
          ${sessionSortDir === "desc" ? "↓" : "↑"}
        </button>
        ${
          selectedCount > 0
            ? html`
                <button class="btn btn-sm sessions-action-btn sessions-clear-btn" @click=${onClearSessions}>
                  ${tr("usageOverview.clearSelection", "Clear Selection")}
                </button>
              `
            : nothing
        }
      ...
      ${
        sessionsTab === "recent"
          ? recentEntries.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${tr("usageOverview.noRecentSessions", "No recent sessions")}</div>
              `
            : html`
          ...
          : sessions.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${tr("usageOverview.noSessionsInRange", "No sessions in range")}</div>
              `
            : html`
        ...
        selectedCount > 1
          ? html`
              <div style="margin-top: 10px;">
                <div class="sessions-card-count">${tr("usageOverview.selected", "Selected")} (${selectedCount})</div>
